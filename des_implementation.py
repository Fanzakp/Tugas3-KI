...
def s_box_substitution(expanded_block):
    ...
    for i in range(8):
        ...
        output.extend([int(x) for x in format(S_BOX[i][row][col], '04b')])
    return output
...
def des_round(left_half, right_half, round_key):
    new_left = right_half
    new_right = xor(left_half, f_function(right_half, round_key))
...
def des_encrypt(plaintext, key):
    ...
    for i in range(0, len(padded_text), 8):
        block = string_to_bit_array(padded_text[i:i+8])
        ...
        ciphertext += bytes(int(''.join(map(str, block[k:k+8])), 2) for k in range(0, 64, 8))
...
def des_decrypt(ciphertext, key):
    ...
    for i in range(0, len(ciphertext), 8):
        block = [int(bit) for byte in ciphertext[i:i+8] for bit in format(byte, '08b')]
        block = permute(block, IP)
        left, right = block[:32], block[32:]
...
def main():
    plaintext = "1234562713y7847829837428981"
    ...
    decrypted = des_decrypt(ciphertext, random_key)
